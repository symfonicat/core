...
function loadProjects() {
    ...
    try {
        ...
        if (Array.isArray(payload?.projects)) {
            const projects = payload.projects
                .map((slug) => normalizeProject({ slug }))
                .filter(Boolean);
            if (projects.length > 0) {
                return dedupeProjects(projects);
            }
...
function sanitizePackageName(slug) {
    const cleaned = slug
        .toLowerCase()
        .replace(/[^a-z0-9-]+/g, '-')
        .replace(/^-+|-+$/g, '');
...
function buildProjectUrl(project) {
    ...
    return `https://${encodedSlug}.${domain}?electron=1`;
}
...
`;
    ...
    const packageName = `symfonicat-${sanitizePackageName(project.slug)}-desktop`;
    const buildConfig = {
        appId: `me.symfonicat.${sanitizePackageName(project.slug)}`,
...
async function main() {
    ...
    const projects = loadProjects().filter((project) => !PROJECT_FILTER || project.slug === PROJECT_FILTER);
    if (projects.length === 0) {
    ...
    for (const project of projects) {
        console.log(`[electron] preparing ${project.slug}`);
        ...
        console.log(`[electron] packaging ${project.slug}`);
