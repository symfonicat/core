...
import (
	...
	brotlicodec "github.com/andybalholm/brotli"
...
func init() {
	...
	caddy.RegisterModule(BrotliDecompress{})
	httpcaddyfile.RegisterHandlerDirective("brotli_precompress", parseCaddyfile)
	httpcaddyfile.RegisterHandlerDirective("brotli_decompress", parseBrotliDecompressCaddyfile)
}
...
type BrotliDecompress struct{}

// /m* requests are always brotli-compressed at the edge before PHP sees them.
...
func (BrotliDecompress) CaddyModule() caddy.ModuleInfo {
	return caddy.ModuleInfo{
		ID:  "http.handlers.brotli_decompress",
		New: func() caddy.Module { return new(BrotliDecompress) },
	}
}
...
func (b *BrotliDecompress) ServeHTTP(w http.ResponseWriter, r *http.Request, next caddyhttp.Handler) error {
	...
	decompressed, err := decompressBrotliBytes(body)
	if err != nil {
		http.Error(w, "invalid brotli request body", http.StatusBadRequest)
		return nil
	}
	...
	r.Header.Del("X-Symfonicat-Content-Encoding")
...
func (b BrotliPrecompress) compressFile(sourcePath string, brotliPath string) error {
	...
	writer := brotlicodec.NewWriterLevel(target, b.Quality)
...
func decompressBrotliBytes(payload []byte) ([]byte, error) {
	reader := brotlicodec.NewReader(bytes.NewReader(payload))
...
func parseBrotliDecompressCaddyfile(h httpcaddyfile.Helper) (caddyhttp.MiddlewareHandler, error) {
	for h.Next() {
	...
	return new(BrotliDecompress), nil
}
