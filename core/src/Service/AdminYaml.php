...
final class AdminYaml implements AdminYamlDumper
{
    ...
    public function load(): array
    {
        ...
        foreach (array_keys($symfonicat) as $table) {
            if ($table === self::TABLE_PREFIX.'admin') {
                continue;
            }

            if (!is_string($table) || !isset($tableSet[$table])) {
                throw new \RuntimeException(sprintf('Unknown Symfonicat core table "%s".', (string) $table));
            }
        }
    ...
    private function orderedTables(AbstractSchemaManager $schemaManager): array
    {
        $tables = array_values(array_filter(
            ...
            static fn (string $table): bool => str_starts_with($table, self::TABLE_PREFIX)
                && !in_array($table, [self::TABLE_PREFIX.'admin', self::TABLE_PREFIX.'core'], true),
        ));
        sort($tables);
