...
{
    private const SESSION_KEY = 'symfonicat_core_mfa';
    private const TARGET_PATH_KEY = 'symfonicat_core_mfa_target';
    ...
    public function rememberTargetPath(Request $request): void
    {
        ...
        if (
            $path === ''
            || str_starts_with($path, '/core/login')
            || str_starts_with($path, '/core/logout')
        ) {
            return;
        }
    ...
    public function consumeTargetPath(Request $request): ?string
    {
        ...
        return is_string($targetPath) && str_starts_with($targetPath, '/core')
            ? $targetPath
            : null;
    }
