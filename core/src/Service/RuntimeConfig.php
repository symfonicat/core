...
final class RuntimeConfig
{
    ...
    public function subdomainByAffixForDomain(string $affix, Domain $domain): ?Subdomain
    {
        $subdomain = $this->subdomainByAffixForDomainLookup($affix, $domain);

        return $subdomain instanceof Subdomain ? $subdomain : null;
    }

    public function subdomainByAffix(string $affix): ?Subdomain
    {
        $affix = $this->normalizeSubdomainAffix($affix);

        return $affix === '' ? null : ($this->catalog()['subdomains_by_lookup'][$this->subdomainKey(null, $affix)] ?? null);
    }

    private function subdomainByAffixForDomainLookup(string $affix, Domain $domain): ?Subdomain
    {
        $affix = $this->normalizeSubdomainAffix($affix);
        if ($affix === '') {
            return null;
        ...
        return $this->catalog()['subdomains_by_lookup'][$this->subdomainKey($domain, $affix)] ?? null;
    }
    ...
    private function catalog(): array
    {
        ...
        foreach ($this->rows($rows, 'symfonicat_subdomain') as $row) {
            ...
            $affix = $this->normalizeSubdomainAffix($row['affix'] ?? '');
            if ($rawId === '' || $affix === '') {
                continue;
            }
            ...
            $subdomainsById[$rawId] = $subdomain;
            $subdomainsByLookup[$this->subdomainKey($domain, $affix)] = $subdomain;
        }
        ...
        foreach ($this->rows($rows, 'symfonicat_module_subdomain') as $row) {
            $module = $modules[(string) ($row['module_id'] ?? '')] ?? null;
            $subdomain = $subdomainsById[(string) ($row['subdomain_id'] ?? '')] ?? null;
        ...
        foreach ($this->rows($rows, 'symfonicat_endpoint') as $row) {
            ...
            $endpoint = (new Endpoint())
                ...
                ->setSubdomain($subdomainsById[(string) ($row['subdomain_id'] ?? '')] ?? null);
        ...
        return $this->catalog = [
            'parcels' => $parcels,
            'domains' => $domains,
            'subdomains' => $subdomainsById,
            'subdomains_by_lookup' => $subdomainsByLookup,
            'modules' => $modules,
    ...
    private function normalizeSubdomainAffix(mixed $affix): string
    {
        $affix = trim((string) $affix, " \t\n\r\0\x0B/");
        if ($affix === '') {
            return '';
        }

        return str_contains($affix, '/') ? substr($affix, strrpos($affix, '/') + 1) : $affix;
    }
    ...
    private function subdomainKey(?Domain $domain, string $affix): string
    {
        ...
        return ($domainId === '' ? '' : $domainId).'|'.$this->normalizeSubdomainAffix($affix);
    }
    ...
    private function sameSubdomain(?Subdomain $left, ?Subdomain $right): bool
    {
        ...
        $leftAffix = $left->getAffix();
        $rightAffix = $right->getAffix();
        if ($leftAffix === null || $rightAffix === null || $leftAffix !== $rightAffix) {
            return false;
        }
