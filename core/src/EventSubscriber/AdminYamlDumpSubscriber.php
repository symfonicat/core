...
final class AdminYamlDumpSubscriber implements EventSubscriber
{
    ...
    public function onFlush(OnFlushEventArgs $args): void
    {
        ...
        $this->shouldDump = $request->attributes->getBoolean(CoreYamlDumpSubscriber::REQUEST_ATTRIBUTE)
            || $this->hasTrackedChanges($args->getObjectManager()->getUnitOfWork());
    }
    ...
    public function postFlush(PostFlushEventArgs $args): void
    {
        ...
        $this->shouldDump = false;
        $request = $this->requestStack->getMainRequest() ?? $this->requestStack->getCurrentRequest();
        $request?->attributes->set('symfonicat_core_yaml_dumped', true);
        $this->adminYaml->dump();
    }
