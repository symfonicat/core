...
{
    #[Route('/core/login', name: 'symfonicat_core_login', methods: ['GET', 'POST'])]
    public function login(Request $request, AdminMfaService $adminMfaService, AuthenticationUtils $authenticationUtils): Response
    {
        ...
        if ($adminMfaService->isVerified($request, $admin)) {
            $targetPath = $adminMfaService->consumeTargetPath($request) ?? $this->generateUrl('symfonicat_core_dashboard');

            return new RedirectResponse($targetPath);
        }

        if ($request->isMethod('POST')) {
            if (!$this->isCsrfTokenValid('symfonicat_core_mfa', (string) $request->request->get('_token'))) {
    ...
    #[Route('/core/login/check', name: 'symfonicat_core_login_check', methods: ['POST'])]
    public function loginCheck(): Response
    {
        return new Response('', Response::HTTP_NO_CONTENT);
    }

    #[Route('/core/logout', name: 'symfonicat_core_logout', methods: ['GET'])]
    public function logout(Request $request, AdminMfaService $adminMfaService): Response
    {
        $adminMfaService->clear($request);

        return new RedirectResponse($this->generateUrl('symfonicat_core_logout_clear'), Response::HTTP_SEE_OTHER, [
            'Cache-Control' => 'no-store, no-cache, must-revalidate, max-age=0',
            'Pragma' => 'no-cache',
            'Expires' => '0',
        ]);
    ...
    #[Route('/core/logout/clear', name: 'symfonicat_core_logout_clear', methods: ['GET'])]
