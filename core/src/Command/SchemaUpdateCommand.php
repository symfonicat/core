    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        try {
            ...
            $subdomainResult = $this->subdomainService->sync($shouldAskForConfirmation ? function (array $subdomainIds) use ($input, $io): bool {
                ...
                $io->listing(array_map(
                    static function (string $subdomainKey): string {
                        [$domainId, $affix] = array_pad(explode('|', $subdomainKey, 2), 2, '');

                        $label = $affix;
                        if ($domainId !== '') {
                            $label .= sprintf(' (%s)', $domainId);
                        }

                        return sprintf('%s from installed package assets', $label);
                    },
                    $subdomainIds,
                ));
        ...
        if ($subdomainResult['created'] !== []) {
            ...
            $io->listing(array_map(
                static function (array $subdomain): string {
                    $label = $subdomain['affix'];
                    if (($subdomain['domain'] ?? '') !== '') {
                        $label .= sprintf(' (%s)', $subdomain['domain']);
                    }
