...
module.exports = function configureSymfonicat(Encore, options = __dirname) {
    ...
    moduleData.projects.forEach((slug) => {
        if (!slug) {
            return;
        }

        const projectPath = path.join(projectDir, 'assets', 'projects', slug, 'index.js');
        if (!fs.existsSync(projectPath)) {
            return;
        }

        Encore.addEntry('projects/' + slug, `./assets/projects/${slug}/index.js`);
    });
    ...
    moduleData.modules.forEach((module) => {
        ...
        const modulePath = path.join(projectDir, 'assets', 'modules', module, 'index.js');
        if (!fs.existsSync(modulePath)) {
            return;
        }

        Encore.addEntry(`modules/${module}`, `./assets/modules/${module}/index.js`);
    });

    Encore
        .enableStimulusBridge('./assets/controllers.json')
        .addEntry('symfonicat', toEntryPath(projectDir, path.join(packageDir, 'assets', 'app.js')))
        .splitEntryChunks()
