...
final class BodyIndentExtension extends AbstractExtension
{
    ...
    public function indentBody(string|Markup $html, int $indent = 16): Markup
    {
        ...
        $raw = $this->formatBodyWithTidy($raw);

        $pad = str_repeat(' ', $indent);
        $lines = preg_split('/\r?\n/', $raw);
        if ($lines === false || $lines === []) {
            return new Markup($raw . "\n", 'UTF-8');
        }
    ...
    private function formatBodyWithTidy(string $html): string
    {
        if (!extension_loaded('tidy')) {
            throw new LogicException('The indent_body filter requires the ext-tidy PHP extension.');
        }
        ...
        $wrapped = "<body>\n{$html}\n</body>";
        $formatted = tidy_repair_string($wrapped, $config, 'utf8');
        if ($formatted === false) {
            throw new RuntimeException('Tidy failed to format the body block output.');
        }
