...
final class BootstrapCommand extends Command
{
    ...
    private function seedLocalDefaults(): string
    {
        ...
        $project = $this->projectRepository->findOneBySlug('project1');
        ...
        if (!$project instanceof Project) {
            ...
            if ($project->getSlug() !== 'project1') {
                $project->setSlug('project1');
                $updatedProject = true;
            }
        }

        $attachedAnalyticsToLocalhost = $this->attachModuleToDomain($localhost, $module);
        $attachedAnalyticsToExample = $this->attachModuleToDomain($exampleDomain, $module);
        $attachedAnalyticsToProject = $this->attachModuleToProject($project, $module);
        ...
        if (
            ...
            if ($attachedAnalyticsToExample) {
                $messages[] = 'attached Analytics to example.com';
            } else {
                $messages[] = 'Analytics already attached to example.com';
            }

            if ($attachedAnalyticsToProject) {
                $messages[] = 'attached Analytics to Project 1';
            } else {
                $messages[] = 'Analytics already attached to Project 1';
            }
